...
export class TwitterAdapter implements PublishingAdapter {
  ...
  constructor() {
    this.mock = new MockPublishingAdapter("twitter");
  }
  ...
  async publish(post: ContentPost, account: PlatformAccount): Promise<PublishResult> {
    ...
    throw new Error("Twitter/X API not configured");
  }

  validate(post: ContentPost): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!post.contenido?.trim()) errors.push("Contenido vacío");
    if ((post.contenido?.length ?? 0) > 280) errors.push("Supera el límite de 280 caracteres");
