...
-- Bloques de contenido para el Manual Operativo Zecamo
-- Compatible con el schema aplicado previamente (sesión anterior de Claude)
-- Idempotente: salta si la sección ya tiene bloques
-- ============================================================
...
'El proceso comercial de Zecamo tiene 9 etapas. Lisandro es el owner del pipeline. El objetivo es **velocidad y calidad**: no dejar leads enfriarse y no cerrar clientes que no deberían ser clientes.');
  ...
  -- Pipeline stages (posiciones 2–10)
  INSERT INTO manual_blocks (section_id, position, type, ref_id)
  SELECT sec_id, 1 + position, 'pipeline_stage', id
  FROM pipeline_stages
  ORDER BY position;

  -- Script discovery (posición 11)
  INSERT INTO manual_blocks (section_id, position, type, ref_id)
  SELECT sec_id, 11, 'script', id
  FROM scripts WHERE type = 'discovery'
  ORDER BY position LIMIT 1;

  -- Script closing (posición 12)
  INSERT INTO manual_blocks (section_id, position, type, ref_id)
  SELECT sec_id, 12, 'script', id
  FROM scripts WHERE type = 'closing'
  ORDER BY position LIMIT 1;

  -- Objection scripts (posiciones 13–16)
  INSERT INTO manual_blocks (section_id, position, type, ref_id)
  SELECT sec_id, 12 + ROW_NUMBER() OVER (ORDER BY position), 'objection', id
  FROM scripts WHERE type = 'objection'
  ORDER BY position;
END $$;
